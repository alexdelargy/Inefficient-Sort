...
public class Main {
	...
	public static ArrayList<Integer> sort(ArrayList<Integer>dataList) {
		ArrayList<Integer>that=dataList;
		while(!checkOrder(that)) {
			int tempIndex=(int)Math.abs(Math.round(Math.random()*that.size()-1));
			int tempElement=that.get(tempIndex);
			int temp2Index=(int)Math.abs(Math.round(Math.random()*that.size()-1));
			int temp2Element=that.get(temp2Index);
			that.set(tempIndex,temp2Element);
			that.set(temp2Index, tempElement);
		}
		return that;
	}
